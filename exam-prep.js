...
async function renderExamCards(kind){
  ...
  for(const file of cfg.examFiles){
    try{ exams.push(await loadJson(`${EXAM_ROOT}/${file}`)); }catch{}
  }
  target.innerHTML=exams.map((exam,i)=>`
    <button class="exam-card" type="button" data-start-exam="${esc(exam.id)}" data-file="${esc(cfg.examFiles[i])}">
    ...
    <div class="exam-runner-grid">
      ...
      <article class="exam-question-card">
        ...
        <div class="exam-runner-actions">
          ${(!sequential && s.index>0)?'<button type="button" class="exam-secondary" data-prev-question>← Anterior</button>':'<span></span>'}
...
async function startExam(file){
  ...
  state.current={exam,file,items:flattenExam(exam),index:0,answers:[]};
  showView(exam.kind);
  renderRunner();
}
...
document.addEventListener('click', async (e)=>{
  ...
  if(nav){
    ...
    showView(kind); renderLanding(kind); return;
  }

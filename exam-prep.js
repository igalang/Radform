...
function examCachedMedia(obj){
  return examMediaDescriptors(obj)
    ...
    .filter(meta => meta?.verified === true && meta?.imageUrl);
}
...
function examMediaHtml(item){
  ...
  if(!media.length){
    const descriptors = [...examMediaDescriptors(item.caseObj), ...examMediaDescriptors(item.question)];
    const exactRequested = descriptors.some(meta => Boolean(meta?.imageFile));
    return exactRequested ? '<div class="exam-image-missing">Esta pregunta tiene una imagen verificada asignada, pero todavía no está disponible en la caché local. Ejecuta “Actualizar biblioteca médica” en GitHub Actions.</div>' : '';
  }
...
function examRenderLanding(kind){
  ...
  const modes = kind === 'ebir'
    ? [
      ...
      ['images','Image Challenge','Solo imágenes abiertas verificadas manualmente',`${images} imágenes verificadas`],
    ]
    : [
  ...
  root.innerHTML = `
    ...
    <div class="exam-image-policy"><strong>Seguridad visual:</strong> los casos con imagen y Image Challenge solo muestran archivos de Wikimedia Commons revisados y aprobados mediante nombre de archivo exacto. Si no existe una imagen adecuada, Radform muestra la pregunta sin imagen.</div>
...
async function examStartSession(kind,component,opts={}){
  ...
  if(!session.items.some(x=>x.type==='question')){
    const root=exam$(`#${kind}ExamRoot`);
    root.insertAdjacentHTML('afterbegin','<div class="exam-load-error">No hay suficientes preguntas disponibles para esta combinación. Prueba otra duración o área. Image Challenge solo utiliza imágenes abiertas verificadas manualmente.</div>');
    return;
  }

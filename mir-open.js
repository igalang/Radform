...
export async function searchCasiMedicos(query, { lengthPerSplit = 35 } = {}) {
  const q = clean(query);
  if (!q) throw new Error('Introduce un término para buscar en preguntas MIR reales.');
  ...
  if (!unique.length && settled.every((part) => part.status === 'rejected')) throw new Error('No se pudo conectar ahora con el banco MIR abierto.');
  return unique;
}
